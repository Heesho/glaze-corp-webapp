...
export async function GET() {
  ...
  try {
    ...
    const [ethRes, btcRes, qrRes, aeroRes, clankerRes] = await Promise.all([
      ...
      fetch("https://api.coingecko.com/api/v3/simple/price?ids=tokenbot-2&vs_currencies=usd"),
    ]);

    const [ethData, btcData, qrData, aeroData, clankerData] = await Promise.all([
      ethRes.json(),
      btcRes.json(),
      qrRes.json(),
      aeroRes.json(),
    ...
    const prices = {
      ...
      clanker: clankerData?.["tokenbot-2"]?.usd ?? 0,
    };
    ...
    return NextResponse.json({ eth: 0, btc: 0, qr: 0, aero: 0, clanker: 0 }, { status: 500 });
  }
}
